...
func generateTransactionBytes() [][]byte {
	...
	ets := []EventTransaction{
		...
		{Name: "s-test-1", UUID: "s1", ParentUUID: "r1", Type: event.Remove},
	}
...
func Test_CreateFileNodeWithTransaction(t *testing.T) {
	...
	assert.Equal(t, "s-test-2-rename", tree.Subs[0].Name, "first sub node name is wrong")
}
...
func Test_RestoreWatcherWithTransactions(t *testing.T) {
	...
	assert.Equal(t, "s-test-2-rename", tw.FileTree.Subs[0].Name, "first sub node name is wrong")
}
